...
<Meta title="Grid/Stuff" component={Grid} />
...
A `PamLayoutGrid` component takes a `PamLayout` and a `data` as props. The `PamLayout` is used to define the columns and the `data` is used to populate the rows.
...
<Canvas>
  <Story name="Some Data">
    <Grid layout={layout} data={data} />
  </Story>
</Canvas>