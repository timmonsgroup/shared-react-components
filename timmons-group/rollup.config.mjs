...
const external = packageJsonFiles.map(file => {
  const packageJson = JSON.parse(fs.readFileSync(file)) || {};
  const peerDependencies = packageJson.peerDependencies || {};
  const dependencies = packageJson.dependencies || {};
  const devDependencies = packageJson.devDependencies || {};

  return Object.keys(peerDependencies).concat(Object.keys(dependencies)).concat(Object.keys(devDependencies));
});

// Now flatten the list and remove the duplicates
const externalList = [...new Set(external.flat())];
